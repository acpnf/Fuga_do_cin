...
def desenhar_chaves(tela, chaves):
    for chave in chaves:
        if not chave[3]:
            try:
                # Carrega a imagem correspondente à cor da chave
                if chave[1] == VERMELHO:
                    img = pygame.image.load('cracha IYODA.png').convert_alpha()
                elif chave[1] == VERDE:
                    img = pygame.image.load('cracha MASSA.png').convert_alpha()
                else:  # AMARELO
                    img = pygame.image.load('cracha SOARES.png').convert_alpha()
                ...
                pos_x = (chave_x - 1) * TAMANHO_CELULA + (TAMANHO_CELULA - img.get_width()) // 2
                pos_y = (chave_y - 1) * TAMANHO_CELULA + (TAMANHO_CELULA - img.get_height()) // 2
                
                # Desenha a imagem
                tela.blit(img, (pos_x, pos_y))
                
            except FileNotFoundError:
                # Fallback para o círculo colorido se a imagem não for encontrada
                chave_x, chave_y = chave[0][1], chave[0][0]
...
def desenhar_jogador(tela, jogador_x, jogador_y):
    try:
        jogador_img = pygame.image.load('MOVIMENTO DIREITA.png').convert_alpha()
