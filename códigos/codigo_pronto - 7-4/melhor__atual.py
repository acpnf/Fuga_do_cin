...
def carregar_imagens_porta():
    try:
        ...
        porta_fechada = pygame.transform.scale(porta_fechada, (TAMANHO_CELULA - 5 , TAMANHO_CELULA - 5))
...
def desenhar_saida(tela, saida, chaves, porta_fechada, porta_aberta):
    if saida:
        ...
        # Posição da porta
        x_pos = (saida[1] - 1) * TAMANHO_CELULA + 10
        y_pos = (saida[0] - 1) * TAMANHO_CELULA + 10
        
        # Desenha a imagem apropriada
        if todas_coletadas:
            tela.blit(porta_aberta, (x_pos, y_pos))
        else:
            tela.blit(porta_fechada, (x_pos, y_pos))
...
def desenhar_placar(tela, fonte_titulo, fonte_texto, chaves):
    ...
    for i, chave in enumerate(chaves):
        ...
        texto = fonte_texto.render(f"{chave['nome']}: {status}", True, PRETO)
        tela.blit(texto, (LARGURA_LAB + 20, 80 + i * 40))
    
    
    # Divisória
    pygame.draw.line(tela, CINZA_ESCURO, (LARGURA_LAB, 220), (LARGURA, 220), 2)
...
def verificar_vitoria(jogador_x, jogador_y, saida, chaves):
    if saida and (jogador_y, jogador_x) == saida and all(chave['coletada'] for chave in chaves):
        return True
    return False
...
def main():
    ...
    # Carrega as imagens da porta uma vez no início
