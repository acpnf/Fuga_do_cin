        ...
        while True:
            ...
            if (x, y) != (1, 1) and all((x, y) != (chave['x'], chave['y']) for chave in chaves):
                chaves.append({
                    ...
                    'frame_atual': 0,
                    'contador_frames': 0,
                    'velocidade_animacao': 8  # Valor fixo para todos os crachás
                })
                break
    return chaves
...
def desenhar_chaves(tela, chaves):
    for chave in chaves:
        if not chave['coletada']:
            ...
            # Posição central (ajustada para o novo tamanho)
...
def desenhar_painel_status(tela, fonte, chaves):
    ...
    for chave in chaves:
        ...
        img_pequena = pygame.transform.scale(chave['imagens'][0], (30, 30))  # Aumentei um pouco
        ...
        tela.blit(texto, (LARGURA - largura_painel + 60, y_offset))  # Ajuste posição
        y_offset += 60  # Aumentei o espaçamento
